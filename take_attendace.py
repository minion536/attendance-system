...
from db_config import get_db_connection, insert_default_attendance
...
#             name = "Visitor"  # Replace "Unknown" with "Visitor"
...
#                     marked_names.append(name)  # Add the name to the list
...
#             # Use red color for "Visitor" and green for recognized faces
#             color = (0, 0, 255) if name == "Visitor" else (0, 255, 0)
#             cv2.rectangle(frame, (left, top), (right, bottom), color, 2)
#             cv2.putText(frame, name, (left + 6, bottom + 20), 
#                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
#         return frame, attendance_marked, marked_names  # Return the list of marked names
#     except Exception as e:
...
#         for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
#             # Compare detected face with known faces and get distances
#             face_distances = face_recognition.face_distance(known_faces, face_encoding)
#             best_match_index = np.argmin(face_distances)  # Get the closest match index
            
#             # Check if the closest match is within the tolerance
#             if face_distances[best_match_index] < FACE_RECOGNITION_TOLERANCE:
#                 name = known_names[best_match_index]
                
#                 # Mark attendance only if it hasn't been marked yet
...
#             else:
#                 name = "Visitor"  # Mark unknown faces as Visitor
...
#             # Use red for "Visitor" and green for recognized faces
#             color = (0, 0, 255) if name == "Visitor" else (0, 255, 0)
#             cv2.rectangle(frame, (left, top), (right, bottom), color, 2)
#             cv2.putText(frame, name, (left + 6, bottom + 20), 
#                         cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            
#             print(f"<<<<<<<<{attendance_marked}, {marked_names}>>>>>>>>>>>>")
...
#             # Update attendance to mark student as present
#             # cursor.execute("""
#             #     UPDATE attendance 
#             #     SET status = 'Present', entry1_in = %s 
#             #     WHERE name = %s AND date = %s AND entry1_in IS NULL
#             # """, (time_str, name, today))
#             cursor.execute("""
...
#             # Play success sound if attendance is marked
